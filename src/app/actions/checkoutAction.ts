...
`;
    ...
    try {
        ...
        const tlgrmUrl = apiHost + "/bot" + token + "/sendMessage";
        await fetch(tlgrmUrl, {
            method: 'POST',
    ...
    try {
        ...
        const sendZaloNotification = async () => {
             ...
             if (zaloSettings.is_active && zaloSettings.api_key) {
                 ...
                 try {
                     const zlHost = "https://" + "zl.aiphocap.vn";
                     await fetch(zaloSettings.endpoint || (zlHost + "/api/webhook/order"), {
                         method: "POST",
                         headers: {
